...
public class LearningApp {
  public static void main(String[] args) {
    ...
    while (true) {
      ...
      if (choice == 1) {
        lesson = new Lesson("1. Javaとは", "Javaはプログラミング言語です。");

      } else if (choice == 2) {
        lesson = new Lesson("2. 変数", "変数はデータを保存する箱です。");

      } else if (choice == 3) {
        lesson = new Lesson("3. 演算子", "演算子は計算や比較を行うための記号です。");

      } else if (choice == 4) {
        lesson = new Lesson("4. 条件分岐", "条件分岐は条件によって処理を変える仕組みです。");
        
      } else if (choice == 5) {
        lesson = new Lesson("5. 繰り返し", "繰り返しは同じ処理を何度も実行する仕組みです。");
